...
import { 
  Plus, Search, Award, Settings, 
  Leaf, Star, Coins, Zap, Info
} from 'lucide-react';
    ...
    <div className="space-y-12 animate-in fade-in duration-700 pb-20">
      <div className="flex flex-col xl:flex-row gap-8 items-center justify-between">
        ...
        <div className="flex gap-4">
          {currentUser && (
            <button onClick={() => setShowSettings(!showSettings)} className="px-8 py-5 bg-ninpo-card border border-white/5 rounded-[1.5rem] text-white font-black text-[12px] uppercase tracking-widest flex items-center gap-3 hover:border-ninpo-lime/40 transition-all shadow-lg active:scale-95">
            ...
            <div className="space-y-6">
               ...
               <div className="flex gap-6 overflow-x-auto no-scrollbar pb-4">
                 {orders.filter(o => o.status !== OrderStatus.DELIVERED && o.status !== OrderStatus.REFUNDED).map(o => (
                   <div key={o.id} className="min-w-[300px] bg-ninpo-midnight p-6 rounded-[2rem] border border-white/5 flex flex-col justify-between shadow-xl">
                      <div>
                        <div className="flex justify-between items-center mb-4">
                          <span className="text-[9px] font-black text-slate-700 uppercase">NODE: {o.id}</span>
                          <span className="text-[9px] font-black text-ninpo-lime uppercase bg-ninpo-lime/5 px-3 py-1 rounded-full border border-ninpo-lime/20">{o.status}</span>
                        </div>
                        <p className="text-white font-black text-xs uppercase truncate mb-1">{o.address}</p>
                      </div>
                      <div className="mt-6 flex items-center gap-4 text-[9px] font-black uppercase text-slate-500">
                         <div className="flex-1 h-1 bg-white/5 rounded-full overflow-hidden">
                           <div className="h-full bg-ninpo-lime shadow-neon transition-all duration-1000" style={{ width: '40%' }} />
                         </div>
                      </div>
                   </div>
                 ))}
        ...
        <div className="max-w-6xl mx-auto space-y-12 animate-in slide-in-bottom">
           <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="bg-white/5 p-8 rounded-[3rem] border border-white/5 space-y-6">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-white font-black uppercase text-[10px] tracking-[0.2em]">Eco Influence</h3>
                        </div>
                        <div className="bg-ninpo-black/80 p-6 rounded-2xl border border-white/5 shadow-xl flex justify-between items-center">
                            <span className="text-white font-black text-3xl tracking-tighter">MI {currentUser?.dailyReturnTotal.toFixed(2)}</span>
                            <div className="group relative"><Info className="w-4 h-4 text-slate-700" /></div>
                        </div>
                    </div>
                 </div>
              </div>
              <div className="bg-ninpo-card p-10 rounded-[4rem] border border-white/5 flex flex-col justify-center items-center text-center space-y-10 shadow-2xl relative overflow-hidden group">
                 <div className="w-24 h-24 bg-ninpo-lime/10 rounded-[2.5rem] flex items-center justify-center border border-ninpo-lime/20 shadow-neon"><Coins className="w-12 h-12 text-ninpo-lime" /></div>
                 <div>
                 ...
                 <button onClick={() => setShowSettings(false)} className="px-6 py-3 bg-white/5 text-slate-500 rounded-xl uppercase text-[9px] font-black hover:text-white transition-colors">Return to Market</button>
              </div>
           </div>
        </div>
      )}
