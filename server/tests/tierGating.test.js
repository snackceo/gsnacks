...
test('GREEN tier gating on fees', async () => {
  ...
  assert.equal(res.body.routeFeeFinal, 4.99);
  assert.equal(res.body.distanceFeeFinal >= 0, true);

  // Enabled GREEN
  stubSettings({ allowGreenTier: true });
  res = await request(app).post('/api/payments/quote').send(baseQuotePayload('U1'));
  assert.equal(res.status, 200);
  assert.equal(res.body.routeFeeFinal, 1);
  assert.equal(res.body.distanceFeeFinal, 0);
});
...
test('PLATINUM free delivery gating', async () => {
  ...
  assert.equal(res.status, 200);
  assert.equal(res.body.routeFeeFinal, 0);
});
...
test('Cash payout normalization by tier eligibility', async () => {
  ...
  assert.equal(res.status, 200);
  // Order should be created with returnPayoutMethod CREDIT
  assert.equal(res.body.sessionUrl?.length > 0, true);
  // We can’t inspect order directly here; instead validate that route succeeds.
  // For stricter checks, we would query the DB, but this test focuses on path coverage.
