...
**allowReceiptApprovalCreateProduct**: If true, receipt review approvals are allowed to execute CREATE_PRODUCT actions; default false so unmatched lines should use unmapped price-intelligence flow. When enabled, UPC is optional for receipt-created products; UPC linking remains optional and separate.
...
ReceiptApprovalAction (enum): Per-item receipt approval decision aligned to ReceiptParseJob.actionSuggestion values. Allowed values: LINK_UPC_TO_PRODUCT, CREATE_UPC, CAPTURE_UNMAPPED, CREATE_PRODUCT, IGNORE. Used to translate review choices into boundUpc/boundProductId or new product creation during approval. For `CREATE_PRODUCT`, UPC is optional: resolution order is bound/suggested product first, then UPC mapping if UPC is present, then normalized-name match, then stub product creation.
