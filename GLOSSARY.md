...
Product: Inventory item definition. Fields: id, optional sku and upc (some products might have one primary UPC stored here), name, price (retail price in dollars), deposit (usually 0.10 if container has deposit), stock (on-hand quantity), sizeOz (volume/size of item), category (e.g., "Soda", "Chips"), image (URL to product image), optional brand, productType (could overlap with category or denote if it's beverage/food), storageZone & storageBin (location codes), isGlass (if container is glass for deposit processing). This model is central for the storefront and inventory management.
